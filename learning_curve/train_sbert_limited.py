...
def train_sbert_limited(run_path, df_train, df_test, df_val, answer_column="text", target_column="label", id_column="id", base_model="all-MiniLM-L6-v2", num_epochs=5, batch_size=8, do_warmup=False, num_pairs_per_example=50, save_model=False):

    num_samples = len(df_train) * num_pairs_per_example
    num_batches_per_round = int(num_samples/batch_size)
    max_steps = num_batches_per_round * num_epochs
    ...
    for _, example_1 in df_val.iterrows():
        for _, example_2 in df_val.iterrows():
