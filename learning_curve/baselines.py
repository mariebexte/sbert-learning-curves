...
def get_predictions(method, df_train, df_val, df_test, id_column="id", answer_column="text", target_column="label"):
    ...
    for idx, test_answer in df_test.iterrows():
        ...
        test_answer = list(copy_eval["text1"])
        if method == "edit":
            copy_eval["compare"] = [-1*distance(ref_answers[i], test_answer[i]) for i in range(len(copy_eval))]
        elif method == "overlap":
            copy_eval["compare"] = [get_overlap_score(ref_answers[i], test_answer[i]) for i in range(len(copy_eval))]
        elif method == "cosine":
            copy_eval["compare"] = [get_overlap_similarity(ref_answers[i], test_answer[i]) for i in range(len(copy_eval))]
        else:
