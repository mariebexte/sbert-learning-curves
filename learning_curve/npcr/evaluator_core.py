...
class Evaluator():
    ...
    def __init__(self, prompt_id, use_char, out_dir, modelname, dev_x, dev_y, yy_dev,test_x, test_y,yy_test, save_model=False):
...
class Evaluator_opti():
    def __init__(self, prompt_id, use_char, out_dir, modelname, features_dev, dev_y_example, dev_y_goal, features_test, test_y_example, test_y_goal, example_size, save_model=False):
        self.use_char = use_char
        self.prompt_id = prompt_id
        ...
        self.out_dir = out_dir
        self.stats_file = os.path.join(out_dir, "stats.tsv")
        with open(self.stats_file, 'w') as stats:
            stats.write("QWK (DEV)\tPRS (DEV)\tSPR (DEV)\tRMSE (DEV)\tBEST_EPOCH (DEV)\n")
        ...
        # For each epoch, put in epoch -> prediction array on test data
        # To be able to later return predictions on best val epoch
    ...
    def print_info(self):
        ...
        with open(self.stats_file, 'a') as stats:
            stats.write(str(self.dev_qwk)+"\t"+str(self.dev_pr)+"\t"+str(self.dev_spr)+"\t"+str(self.dev_rmse)+"\t"+str(self.best_dev_epoch)+"\n")
    ...
    def print_final_info(self):
        ...
        best_epoch = self.best_dev_epoch
        # test_predictions_on_best_val_epoch = self.test_predictions[best_epoch]

        best_model = torch.load(self.out_dir + '/dev' + self.modelname)
        best_model.eval()
        test_pred_int = []

        with torch.no_grad():
            for step, (batch_test_x0, batch_test_x1, batch_example_s) in enumerate(self.test_loader):
                test_y_pred = best_model(batch_test_x0.cuda(), batch_test_x1.cuda())
                ...
                test_pred_int.append(test_pred_i.item())
            # test_pred_int = np.array(test_pred_int)

        with open(self.stats_file, 'a') as stats:
            stats.write("\nBEST EPOCH:\t"+str(best_epoch)+"\n")

        os.remove(self.out_dir + '/dev' + self.modelname)
        print("QWK with their code: ", kappa(self.test_y_goal, test_pred_int, "quadratic"))
    ...
    def evaluate(self, model, epoch, print_info=False):
        ...
        # test_pred_int = np.array(test_pred_int)
        # self.test_predictions[epoch] = test_pred_int.tolist()

        # print("EVALUATION ON TEST DONE IN "+str(datetime.now()-start))
        # start = datetime.now()

        self.calc_correl(None, dev_pred_int, None)
        self.calc_kappa(None, dev_pred_int, None)
        self.calc_rmse(None, dev_pred_int, None)

        print("DEV PREDS: ", dev_pred_int)
        print("DEV GOLD", self.dev_y_goal)
