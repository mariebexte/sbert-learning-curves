            ...
            if os.path.exists(results_path):
                ...
                if method in ["SBERT", "edit", "overlap", "cosine", "pretrained"]:
                    ...
                    if os.path.exists(results_path_max):
                        ...
                        plt.plot(method_results_max.columns, method_results_max_avg, "o", color=colors[method], linestyle=strategies["max"], label=method)
...
def strategy_curve(strategy_path, dataset_name, eval_measure):
    ...
    for method in method_results_dict:
        ...
        linestyle = strategies["avg"]
        if method.endswith("_max"):
            linestyle = strategies["max"]
        plt.plot(method_overview_df.columns, method_avg, "o", color=colors[method], linestyle=linestyle, label=method)
