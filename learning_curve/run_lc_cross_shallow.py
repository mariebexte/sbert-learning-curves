...
def run_cross_shallow(results_folder, dataset_name, prompt_name, shallow_cross_data, train_path, val_path, test_path, method, eval_measure, sampling_strategy, num_labels, max_size=None, upsample_training=False, num_samples=20, predetermined_train_sizes=None, bert_base="bert-base-uncased", sbert_base="all-MiniLM-L6-v2", shallow_cross_method='merge'):

    target_path = os.path.join(results_folder, dataset_name, sampling_strategy, prompt_name, method+"_"+shallow_cross_method)
        ...
        for train_size in train_sizes:
            ...
            for sample in range(num_samples):
                ...
                if method in ["LR", "RF", "SVM"]:
                    y_pred = train_shallow_cross(method=method, df_cross=shallow_cross_data, df_train=df_train_subset, df_val=df_val, df_test=df_test, cross_method=shallow_cross_method)
