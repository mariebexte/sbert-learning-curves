...
colors = {"BERT_old": "green", "BERT_new": "orange", "BERT_old_save2": "green", "BERT_new_save2": "blue", "SBERT_noval": "deepskyblue", "BERT_10e": "red", "BERT_oldConfig": "red", "SBERT_wrongVal": "lime", "SBERT_wrongVal_max": "deepskyblue", "SBERT_valFixed": "lime", "SBERT_max": "pink", "SBERT": "red", "LR": "blue", "RF": "green", "SVM": "orange", "BERT": "purple",
          ...
          "edit": "red", "overlap": "blue", "cosine": "orange", "pretrained": "pink"}
...
def prompt_curve(prompt_dir, dataset_name, sampling_strategy, eval_measure):
        ...
        if os.path.isdir(os.path.join(prompt_dir, method)):
            ...
            if method in ["LR", "BERT"]:
                ...
                if os.path.exists(results_path):
                    ...
                    x_values = [int(training_size) for training_size in method_results.columns]

                    # x_values = x_values[10:]
                    # method_results_avg = method_results_avg[10:]
                    x_values = x_values[:10]
                    method_results_avg = method_results_avg[:10]

                    plt.plot(x_values, method_results_avg, "o-", color=colors[method], label=method)
                    ...
                    #         x_values = [int(training_size) for training_size in method_results_max.columns]
                    #         plt.plot(x_values, method_results_max_avg, "o-", color=colors[method], label=method)
    ...
    plt.savefig(os.path.join(prompt_dir, dataset_name+"_"+sampling_strategy+"_"+prompt_name+"_method-comparison_DE.png"))
 
    plt.clf()
    plt.cla()
    plt.close()

    prompt_results.to_csv(os.path.join(prompt_dir, dataset_name+"_"+sampling_strategy+"_"+prompt_name+"_method-comparison_DE.csv"), index=None)
...
def strategy_curve(strategy_path, dataset_name, eval_measure):
    ...
    for method in method_results_dict:
        ...
        x_values = [int(training_size) for training_size in method_overview_df.columns]
        plt.plot(x_values, method_avg, "o-", color=colors[method], label=method)
